  ...
  return (
    <>
      ...
      <main className="pt-20 pb-16 md:pb-0">
        ...
        {/* Service Photo Gallery — До / Процесс / После */}
        {(() => {
          const SERVICE_GALLERY: Record<string, Array<{ src: string; title: string; desc: string; badge: string; badgeColor: string }>> = {
            'borba-s-krotami': [
              ...
              <div className="container mx-auto px-4">
                <AnimatedSection animation="fade-up">
                  <SectionHeading
                    ...
                    subtitle={subtitleMap[service.slug] || ''}
                  />
                </AnimatedSection>
                    ...
                    <AnimatedSection key={idx} animation="fade-up" delay={idx * 150}>
                      <Card className="overflow-hidden h-full hover:shadow-xl transition-shadow">
                        <div className="relative aspect-[4/3]">
                          <img src={item.src} alt={item.title} width={400} height={300} className="w-full h-full object-cover" loading="lazy" decoding="async" />
                          <span className={`absolute top-3 left-3 px-3 py-1 rounded-full text-xs font-bold ${item.badgeColor}`}>{item.badge}</span>
                        </div>
