...
В стоимость входит: диагностика, обработка, консультация по профилактике и гарантия до 1 года.
    `,
    intentType: 'guide' as const,
    tldrGenerator: (pest: typeof pests[0]) => [
      `${pest.namePlural} — ${pest.shortDescription}`,
      `Признаки: ${pest.signs.slice(0, 2).join(', ')}`,
      `Методы: ${pest.methods.join(', ')}`,
      `Результат: ${pest.timeToResult}`,
      `Стоимость: от ${pest.priceFrom}₽ за 1-комнатную квартиру`,
    ],
...
Результат: ${pest.timeToResult}
    `,
    intentType: 'symptoms' as const,
    tldrGenerator: (pest: typeof pests[0]) => [
      `${pest.namePlural} в квартире — ${pest.signs[0]}`,
      `Пути проникновения: вентиляция, б/у мебель, щели, с одеждой`,
      `Стоимость обработки: от ${pest.priceFrom}₽`,
      `Результат: ${pest.timeToResult}`,
    ],
    faqGenerator: (pest: typeof pests[0]) => [
...
Профессиональная обработка — самый надёжный способ избавиться от ${pest.genitive}. Стоимость от ${pest.priceFrom}₽.
    `,
    intentType: 'prevention' as const,
    tldrGenerator: (pest: typeof pests[0]) => [
      `Основные пути: от соседей, с вещами, из подвалов, с улицы`,
      `Привлекают: пища, вода, тепло, антисанитария`,
      `Профилактика: ${pest.prevention.slice(0, 2).join('; ')}`,
    ],
    faqGenerator: (pest: typeof pests[0]) => [
...
Если вы заметили хотя бы несколько особей ${pest.genitive}, не теряйте время на эксперименты. Профессиональная обработка от ${pest.priceFrom}₽ решит проблему за ${pest.timeToResult}.
    ...
    intentType: 'guide' as const,
    tldrGenerator: (pest: typeof pests[0]) => [
      `Борная кислота — средняя эффективность, результат через 2-3 недели`,
      `Эфирные масла, уксус — только отпугивают, не уничтожают`,
      `Народные методы не действуют на яйца и личинки`,
      `Профессиональная обработка от ${pest.priceFrom}₽ — результат за ${pest.timeToResult}`,
    ],
    faqGenerator: (pest: typeof pests[0]) => [
...
:::
    ...
    tldrGenerator: (pest: typeof pests[0]) => [
      `Методы: ${pest.methods.join(', ')}`,
      `Этапы: диагностика → подготовка → обработка (40-90 мин) → контроль`,
      `Стоимость: от ${pest.priceFrom}₽ (1-комнатная)`,
      `Гарантия: до 1 года, повторный выезд бесплатно`,
      `Препараты IV класса опасности — безопасны после проветривания`,
    ],
    faqGenerator: (pest: typeof pests[0]) => [
...
:::
    ...
    tldrGenerator: (pest: typeof pests[0]) => [
      `Профилактика: ${pest.prevention.slice(0, 2).join('; ')}`,
      `Рекомендуется 1-2 раза в год, особенно весной и осенью`,
      `Стоимость профилактики: от ${Math.round(pest.priceFrom * 0.8)}₽`,
    ],
    faqGenerator: (pest: typeof pests[0]) => [
...
При обнаружении ${pest.genitive} действуйте быстро. Профессиональная обработка от ${pest.priceFrom}₽.
    `,
    intentType: 'health-risk' as const,
    tldrGenerator: (pest: typeof pests[0]) => [
      `Уровень опасности: ${pest.dangerLevel === 'high' ? 'высокий' : pest.dangerLevel === 'medium' ? 'средний' : 'низкий'}`,
      `Угрозы: инфекции, аллергия, порча имущества`,
      `Группы риска: дети, пожилые, аллергики`,
      `Обработка: от ${pest.priceFrom}₽`,
    ],
    faqGenerator: (pest: typeof pests[0]) => [
...
Это нормально в первые дни — препарат действует постепенно. Если через 2 недели проблема сохраняется, звоните — проведём бесплатную повторную обработку по гарантии.
    `,
    intentType: 'howto' as const,
    tldrGenerator: (pest: typeof pests[0]) => [
      `Покинуть помещение на 2-4 часа после обработки`,
      `Проветрить 30-60 минут, протереть контактные поверхности`,
      `Не мыть полы 3-5 дней`,
      `Результат: ${pest.timeToResult}, полное уничтожение — 2-3 недели`,
    ],
    faqGenerator: (pest: typeof pests[0]) => [
...
Точную стоимость можно узнать по телефону или после бесплатного осмотра помещения.
    `,
    intentType: 'price' as const,
    tldrGenerator: (pest: typeof pests[0]) => [
      `Квартира до 50 м²: от ${pest.priceFrom}₽`,
      `Частный дом: от ${Math.round(pest.priceFrom * 1.5)}₽`,
      `В стоимость входит: диагностика, препараты, гарантия до 1 года`,
      `Цена зависит от площади, степени заражения и метода`,
    ],
    faqGenerator: (pest: typeof pests[0]) => [
...
export const pestsArticles: BlogArticle[] = insectPests.flatMap((pest, pestIndex) =>
  articleTemplates.map((template, templateIndex) => {
    ...
    return {
      ...
      tldr: template.tldrGenerator(pest),
      intent: template.intentType,
    };
  })
);
