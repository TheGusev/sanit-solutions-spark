...
const pestIntroTemplates = [
  ...
  (name: string, genitive: string) => 
    `${name} могут появиться в любом помещении, независимо от уровня чистоты и благополучия района. Опытные специалисты ООО «Санитарные Решения» готовы помочь вам избавиться от ${genitive} быстро, эффективно и с гарантией результата.`,
...
const premisesIntroTemplates = [
  ...
  (objectName: string, genitive: string) => 
    `Профессиональная обработка ${genitive} от вредителей включает использование современных препаратов и оборудования. Специалисты ООО «Санитарные Решения» имеют многолетний опыт работы с ${genitive} различных типов.`,
